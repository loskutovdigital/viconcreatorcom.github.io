...
def footer(lang):
 ...
 nav=''.join(f'<a href="{url(k,lang)}">{e(v)}</a>' for k,v in [('about',t['about']),('prices',t['prices']),('locations',t['locations']),('contact',pick(('Get in touch','Neem contact op'),lang))])
...
def cards(lang):
 return '<div class="portfolio-grid">'+''.join(f'<a class="portfolio-card" href="{url(k,lang)}"><div class="card-image">{photo(v["card"],lang,sizes="(max-width: 760px) 44vw, 29vw")}</div><div class="card-meta"><h3>{e(pick(v["label"],lang))}</h3><span>0{n+1}</span></div><p class="card-desc">{e(pick(v["tag"],lang))}</p></a>' for n,(k,v) in enumerate(SERVICES.items()))+'</div>'
...
def page(key,lang,title,description,body,theme='cream',hero=None,service=None):
 ...
 org={'@type':'ProfessionalService','@id':ORIGIN+'/#business','name':'Vicon Creator','legalName':'Vicon Creator','url':ORIGIN+'/', 'founder':{'@type':'Person','name':NAME},'identifier':{'@type':'PropertyValue','propertyID':'KVK','value':'97270725'},'areaServed':[{'@type':'Country','name':'Netherlands'},{'@type':'Country','name':'Belgium'}],'sameAs':[INSTAGRAM]}
 graph=[org,{'@type':'WebSite','@id':ORIGIN+'/#website','name':'Viktoriia Loskutova · Vicon Creator','url':ORIGIN+'/','inLanguage':['en','nl'],'publisher':{'@id':ORIGIN+'/#business'}},{'@type':'WebPage','@id':u+'#page','url':u,'name':title,'description':description,'inLanguage':lang,'isPartOf':{'@id':ORIGIN+'/#website'}}]
 ...
 text=f'''<!doctype html><html lang="{lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{e(title)}</title><meta name="description" content="{e(description)}"><meta name="robots" content="{robots}"><meta name="author" content="{NAME}"><meta name="theme-color" content="#20271f"><link rel="canonical" href="{u}">{alternates}<meta property="og:type" content="website"><meta property="og:title" content="{e(title)}"><meta property="og:description" content="{e(description)}"><meta property="og:url" content="{u}"><meta property="og:site_name" content="Vicon Creator"><meta property="og:locale" content="{ 'en_GB' if lang=='en' else 'nl_NL' }"><link rel="icon" href="/favicon.svg" type="image/svg+xml">{preload}<link rel="stylesheet" href="/assets/site.css"><script src="/assets/site.js" defer></script><script type="module" src="/assets/booking.mjs"></script><script type="application/ld+json">{json.dumps({'@context':'https://schema.org','@graph':graph},ensure_ascii=False).replace('</','<\\/')}</script></head><body data-theme="{theme}" data-session="{service or ""}">{header(key,lang)}<main id="main">{body}</main>{footer(lang)}{lightbox(lang)}{booking_dialog(lang)}</body></html>'''
...
def home(lang):
 ...
 <section class="wrap section" style="padding-top:0"><div class="section-heading"><div><span class="eyebrow">{pick(('Six ways to tell a story','Zes manieren om een verhaal te vertellen'),lang)}</span><h2>{pick(('The work.','Het werk.'),lang)}</h2>{link('portfolio',pick(('View all work','Bekijk alles'),lang),lang)}</div>{cards(lang)}</section>
 ...
 page('home',lang,pick(('Viktoriia Loskutova | Cinematic Photographer Netherlands','Viktoriia Loskutova | Filmische fotograaf Nederland'),lang),pick(('Wedding, couples, maternity, portrait, family and event photography by Vicon Creator. Based in the Netherlands, photographing stories across the Netherlands.','Trouwfotografie, loveshoots, zwangerschapsfoto’s, portretten, gezins- en evenementenfotografie door Vicon Creator. Vanuit Nederland, door heel Nederland.'),lang),body,hero='lovestory-06')
...
def about(lang):
 ...
 <section class="section about-story wrap"><div><span class="eyebrow">{pick(('How I see it','Hoe ik het zie'),lang)}</span><h2>{pick(('A photograph<br>should feel<br><em>like something.</em>','Een foto<br>mag iets<br><em>laten voelen.</em>'),lang)}</h2></div><div class="story-body"><p>{pick(('Beautiful light matters. So does the person standing in it. My approach gives both room: an intentional composition, a little guidance, and enough freedom for something unplanned to happen.','Mooi licht doet ertoe. De persoon die erin staat ook. Mijn aanpak geeft beide ruimte: een bewuste compositie, een beetje begeleiding en genoeg vrijheid voor iets onverwachts.'),lang)}</p><p>{pick(('I photograph weddings and couples, individual portraits, maternity, children, families and events. The pace changes with each session, but the attention stays the same. I look for photographs with atmosphere and a clear sense of the people in them.','Ik fotografeer bruiloften en koppels, individuele portretten, zwangerschappen, kinderen, gezinnen en evenementen. Het tempo verandert per sessie, maar de aandacht blijft hetzelfde. Ik zoek beelden met sfeer en een duidelijk gevoel voor de mensen erin.'),lang)}</p><p>{pick(('Editing is part of that vision. Warm colour, considered contrast and expressive black and white help the photographs belong together, while keeping the feeling of the moment.','Bewerking hoort bij die visie. Warme kleuren, zorgvuldig contrast en expressief zwart-wit brengen samenhang in de foto’s, terwijl het gevoel van het moment blijft.'),lang)}</p><div class="signature">Viktoriia</div></div></section>
 <div class="wide-interlude">{photo('lovestory-09',lang,sizes='100vw')}</div>
 <section class="section wrap"><div class="section-heading"><div><span class="eyebrow">{pick(('Find your chapter','Vind jouw hoofdstuk'),lang)}</span><h2>{pick(('What shall we make?','Wat gaan we maken?'),lang)}</h2></div><div class="region-links">{''.join(link(k,pick(v['label'],lang),lang) for k,v in SERVICES.items())}</div></section>{cta(lang)}'''
...
def contact(lang):
 t=UI[lang]
 checklist=pick(([('The chapter','A wedding, love story, portrait, maternity, family session or event.'),('The when & where','Your preferred date, city or venue. A rough idea is welcome too.'),('What you have in mind','The atmosphere, the people involved and anything you want me to know.')],[('Het hoofdstuk','Een bruiloft, loveshoot, portret, zwangerschapsshoot, gezinssessie of evenement.'),('Het moment & de plek','Je gewenste datum, stad of locatie. Een globaal idee is ook welkom.'),('Wat je voor ogen hebt','De sfeer, wie erbij zijn en alles wat je me wilt vertellen.')]),lang)
...
def privacy(lang):
 t=UI[lang]
 body=f'''<div class="wrap page-top">{breadcrumb(t['privacy'],lang)}<section class="page-heading"><span class="eyebrow">Vicon Creator</span><h1>{pick(('Privacy &<br>business details.','Privacy &<br>bedrijfsgegevens.'),lang)}</h1></section><article class="editorial-body" style="padding-bottom:100px"><h2>{pick(('Business','Bedrijf'),lang)}</h2><p>Vicon Creator<br>Viktoriia Loskutova<br>KVK: 97270725<br>{pick(('Legal form: sole proprietorship','Rechtsvorm: eenmanszaak'),lang)}<br>{pick(('Branch number','Vestigingsnummer'),lang)}: 000062524429<br>Netherlands</p><h2>{pick(('Visiting the portfolio','Het portfolio bezoeken'),lang)}</h2><p>{pick(('The portfolio itself does not set analytics or advertising cookies. Photographs, videos and fonts are served with the site. Hosting and any private-access sign-in may process technical request or account data to deliver and protect the site.','Het portfolio zelf plaatst geen analytische of advertentiecookies. Foto’s, video’s en lettertypen worden via de site geladen. De hosting en een eventuele aanmelding voor privétoegang kunnen technische verzoek- of accountgegevens verwerken om de site te leveren en te beschermen.'),lang)}</p><h2>{pick(('Getting in touch','Contact opnemen'),lang)}</h2><p>{pick(('The enquiry form asks for your name, session type, preferred date if provided, contact method and contact details. If you choose Other, it also asks for your idea. These details, the page address and submission time are sent through the website’s existing Google Apps Script enquiry service and notification workflow so the website administrator can arrange a reply. They are not published on the site or saved in browser storage. The selected contact method is how you would like to receive a reply; it does not change how the form is submitted. You can also contact @vicon.creator directly on Instagram, where Instagram’s own privacy settings and policies apply.','Het aanvraagformulier vraagt om je naam, het soort fotoshoot, een eventuele voorkeursdatum, contactmethode en contactgegevens. Kies je Anders, dan vragen we ook naar je idee. Deze gegevens, het pagina-adres en het verzendtijdstip worden via de bestaande Google Apps Script-aanvraagservice en meldingsroute verwerkt, zodat de websitebeheerder een reactie kan regelen. Ze worden niet op de site gepubliceerd of in browseropslag bewaard. De gekozen contactmethode bepaalt hoe je graag een reactie ontvangt, niet hoe het formulier wordt verstuurd. Je kunt ook rechtstreeks contact opnemen met @vicon.creator op Instagram. Daar gelden de privacyinstellingen en het beleid van Instagram.'),lang)}</p><h2>{pick(('Photographs & use','Foto’s & gebruik'),lang)}</h2><p>{pick(('The photographs and films in this portfolio are the work of Viktoriia Loskutova / Vicon Creator. Displaying them here does not grant permission to reuse or publish them. For usage requests or a concern about a photograph, please contact the photographer.','De foto’s en films in dit portfolio zijn het werk van Viktoriia Loskutova / Vicon Creator. De weergave hier geeft geen toestemming om ze opnieuw te gebruiken of te publiceren. Neem voor gebruiksverzoeken of vragen over een foto contact op met de fotograaf.'),lang)}</p><a class="text-link" href="{INSTAGRAM}" target="_blank" rel="noopener noreferrer">@vicon.creator {arrow()}</a></article></div>'''
...
(DIST/'404.html').write_text(f'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex"><title>Page not found | Vicon Creator</title><link rel="stylesheet" href="/assets/site.css"></head><body>{header("home","en")}<main id="main" class="empty-page wrap"><span class="eyebrow">404 · Vicon Creator</span><h1>A different<br><em>way back.</em></h1><p>This page could not be found.<br>Deze pagina bestaat niet.</p>{link("home","Return to the photographs","en","button")}</main>{footer("en")}{booking_dialog("en")}<script src="/assets/site.js" defer></script><script type="module" src="/assets/booking.mjs"></script></body></html>')
